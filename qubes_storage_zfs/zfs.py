...
class ZFSQPool(qubes.storage.Pool):
    ...
    *zpool*/*dataset* arguments. ZFS zvol naming scheme:
    ...
    def __init__(self, name:str, block_devices=None, **kwargs):
    ...
    def init_volume(self, vm, volume_config, zfs_ns:str=''):
    ...
    def setup(self):
        ...
        for namespace in [b"import", b"vm"]:
            try:
                ...
                raise qubes.storage.StoragePoolException(
                    "ZFS dataset for {!s}/{!s} already exists".format(
                        self.name, namespace
                    )
                )
            except libzfs_core.exceptions.ZFSError as e:
                raise qubes.storage.StoragePoolException(
                    "ZFS dataset {!s}/{!s} could not be created: {!r}".format(
